...
# ## 📝 참고 사항
# 
# ### Kaggle 학습 Output 파일 업로드 방법
# 1. Kaggle 노트북 실행 후 Output 탭에서 `models/` 폴더 다운로드
# 2. Google Drive `25WinterProject/models/production/` 폴더에 업로드
# 3. 필요한 파일들:
#    - `cnnlstm_f0.pth` ~ `cnnlstm_f4.pth`
...
# 
# ### CatBoost 모델
# - Kaggle 학습에서 CatBoost는 별도 저장되지 않습니다
# - 최초 실행 시 Supabase 데이터로 자동 학습됩니다
# - 이후 `catboost_model_v7e.cbm`으로 저장되어 재사용됩니다
# 
# ### Supabase 테이블 스키마
# ```sql
# -- predictions 테이블
